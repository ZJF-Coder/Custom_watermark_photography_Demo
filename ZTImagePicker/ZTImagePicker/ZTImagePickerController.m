...
#define defineWeakSelf __weak typeof(self) weakSelf = self;     //(弱引用)
#define StrongSelf __strong typeof(self) strongSelf = weakSelf; //(强引用)

@interface ZTImagePickerController () <UIGestureRecognizerDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)p_initAVCaptureSession{
    ...
    self.preview.openBlock = ^{
        [weakSelf OpenPhotoLibary];
    };
...
- (void)p_configActions{
    ...
    [self.preview.photoLibraryButton addTarget:self action:@selector(OpenPhotoLibary) forControlEvents:UIControlEventTouchUpInside];
...
//    NSString *base64Data = [imagedata base64EncodedStringWithOptions:NSDataBase64Encoding64CharacterLineLength];
    ...
    [picker dismissViewControllerAnimated:NO completion:^{
        if ([self.delegate respondsToSelector:@selector(backWithImage:)]) {
            [self.delegate backWithImage:originalImage];
        }
    }];

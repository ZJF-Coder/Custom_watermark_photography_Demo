...
- (UIImage *)markedImageWithType:(XBWaterMark )waterMarkType date:(NSDate *)date user:(NSString *)user addressString:(NSString *)addressString actionString:(NSString *)actionString placLocation:(ZTLocationModel *)locationModel withPhone:(NSString *)phone xmType:(NSString *)xmType{
    ...
    UIView *waterMarkView = [self p_markWaterMarkView:waterMarkType date:date user:user addressString:addressString actionString:actionString placLocation:locationModel withPhone:phone newSize:newSize xmType:xmType];
    ...
    for (UIView *subView in waterMarkView.subviews) {
...
- (UIView *)p_markWaterMarkView:(XBWaterMark )waterMarkType date:(NSDate *)date user:(NSString *)user addressString:(NSString *)addressString actionString:(NSString *)actionString placLocation:(ZTLocationModel *)locationModel withPhone:(NSString *)phone newSize:(CGSize )newSize xmType:(NSString *)xmType{
    UIView *waterMarkView = nil;
    if(waterMarkType == XBWaterMarkDepartMent){
        XBWaterTypeWorkReportView *reportView = [[XBWaterTypeWorkReportView alloc] initWithFrame:CGRectMake(0, 0, newSize.width, newSize.height)];
        [reportView layoutSubviews];
        waterMarkView = reportView;
        
    }else if(waterMarkType == XBWaterMarkDefault){
        XBWaterTypeHandleProblemView *problemView = [[XBWaterTypeHandleProblemView alloc] initWithFrame:CGRectMake(0, 0, newSize.width, newSize.height)];
        XBWaterMarkHandleProblemModel *model = [XBWaterMarkHandleProblemModel new];
        model.userName = user;
        model.address = addressString;
        model.action = actionString;
        model.date = date;
        problemView.problemModel = model;
...
- (void)saveImageToAlarmPhotoAlbum {
    [self saveImageToAlarmPhotoAlbumWithQuality:0.5f];
}
